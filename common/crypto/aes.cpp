...
std::string aes::encrypt(const std::string& plaintext, const std::string& skey)
{
    aes_context ctx;
    ...
    zbase64::base64_encode(output, outlen, basebuf);
...
std::string aes::decrypt(const std::string& basestr, const std::string& skey)
{
    aes_context ctx;
    uint8 key[32] = { 0 };
    memcpy(key, skey.c_str(), skey.length());
    ...
    outlen = zbase64::base64_decode(basestr.c_str(), basestr.length(), output);
